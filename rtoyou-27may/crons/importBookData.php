...
$handle = fopen("RTOData-BOOK.csv","r");
$count = 0;
...
while (($fields = fgetcsv($handle, 0, ",")) !== FALSE) {
    ...
    if(!empty($review)){
        ...
        $reviews[]=array('comment'=>$review,'comment_from'=>$allUsersDb[$user]['user_id'],'cat_id'=>$hospital,'rating'=>$rating,'status'=>1,'comment_time'=>date('Y-m-d H:i:s'),'comment_title'=>substr($r,0 ,strpos($r,"\n")));
    }
    ...
    if(empty($subcat)){
        if(!empty($subcatName)) {
            ...
            $subcat['category_link_id'] = $hospital;
    ...
    if(empty($fields[0]) && empty($fields[1]) && empty($fields[2]) && empty($fields[3]) && empty($fields[4]) && empty($fields[5]) && empty($fields[6])) {
