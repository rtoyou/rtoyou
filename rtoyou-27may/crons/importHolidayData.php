...
while (($fields = fgetcsv($handle, 0, ",")) !== FALSE) {
    ...
    if(!empty($review)){
        ...
        $reviews[]=array('comment'=>$review,'comment_from'=>$allUsersDb[$user]['user_id'],'cat_id'=>$holiday,'rating'=>$rating,'status'=>1,'comment_time'=>date('Y-m-d H:i:s'),'comment_title'=>substr($r,0 ,strpos($r,"\n")));
    }
    ...
    if(empty($subcat)){
        if(!empty($subcatName)) {
            ...
            $subcat['category_link_id'] = $holiday;
