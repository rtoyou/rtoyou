    ...
    {        
        ...
        if($lastpage > 0){
			...
			if($showResultsOfTotal){
				...
				$output .= CHtml::closeTag('span').self::NL;       
			}			

            if($lastpage > 1){			
                $output .= CHtml::openTag('ul', array('class'=>'pagination margin-top-none'));
                // draw previous button
                if(in_array($paginationType, array('fullnumbers', 'prevnext', 'oldernewer'))){
                    if($page > 1){
                        $output .= "<li>".CHtml::link('&laquo; '.$wPrevious, $actionPath.(($linkType) ? '/page/'.$prev : $paramsSign.'page='.$prev), array('class'=>'first-link'))."</li>";
                    }else{
                        if($showEmptyLinks) $output .= CHtml::tag('li', array('class'=>'disabled'), '<a href="javascript:void(0);">&laquo; '.$wPrevious."</a>");
                    }
                }
                ...
                if($lastpage < 7 + ($adjacents * 2)){	
                    ...
                    for($counter = 1; $counter <= $lastpage; $counter++){
                        if($counter == $page){
                            $middlePart .= CHtml::tag('li', array('class'=>'active'), "<a href='javascript:void(0);'>".$counter."</a>");
                        }else{
                            $middlePart .= "<li>".CHtml::link($counter, $actionPath.(($linkType) ? '/page/'.$counter : $paramsSign.'page='.$counter))."</li>";
                        }
                    }
                // enough pages to hide some    
                    ...
                    if($page < 1 + ($adjacents * 2)){
                        ...
                        $middlePart .= '...';
                        $middlePart .= "<li>".CHtml::link($lpm1, $actionPath.(($linkType) ? '/page/'.$lpm1 : $paramsSign.'page='.$lpm1))."</li>";
                        $middlePart .= "<li>".CHtml::link($lastpage, $actionPath.(($linkType) ? '/page/'.$lastpage : $paramsSign.'page='.$lastpage))."</li>";
                    }
                    // in middle, hide some front and some back
                    else if($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2)){
                        $middlePart .= "<li>".CHtml::link('1', $actionPath.(($linkType) ? '/page/1' : $paramsSign.'page=1'))."</li>";
                        $middlePart .= "<li>".CHtml::link('2', $actionPath.(($linkType) ? '/page/2' : $paramsSign.'page=2'))."</li>";
                        $middlePart .= '...';
                ...
                if(in_array($paginationType, array('fullnumbers', 'prevnext', 'oldernewer'))){    
                    if($page < $counter - 1){
                        $output .= "<li>".CHtml::link($wNext.' &raquo;', $actionPath.(($linkType) ? '/page/'.$next : $paramsSign.'page='.$next), array('class'=>'last-link'))."</li>";
                    }else{
                        if($showEmptyLinks) $output .= CHtml::tag('li', array('class'=>'disabled'), "<a href='javascript:void(0);'>".$wNext.' &raquo;</a>');
                    }
                }
