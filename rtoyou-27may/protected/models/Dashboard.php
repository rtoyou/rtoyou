...
class Dashboard extends CModel
{
	...
	*/
	...
	public function getRecentComments($limit=4) {
	...
	public function getReviewOfDay($date = '' ) {
        ...
        $review = $this->_db->select("select r1.* ,rp.user_name , scat.subcat_name , scat.category_seo,cat.category_seo as catSeo, scat.subcat_id from rtoyou_reviews r1 , ".CConfig::get('db.prefix')."user_profile rp ,".CConfig::get('db.prefix')."subcategory scat,".CConfig::get('db.prefix')."category cat  where r1.review_points = (select MAX(review_points) 
            from rtoyou_reviews r2 where r2.status = 1 and DATE_FORMAT(r2.comment_time,'%Y-%m-%d') = :currentdate ) AND scat.subcat_id = r1.subcat_id and rp.user_id = r1.comment_from and scat.category_link_id = cat.cat_id and r1.status = 1 ORDER BY comment_time DESC LIMIT 1",array('currentdate'=>$date));
