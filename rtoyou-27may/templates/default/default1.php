...
<head>
    ...
    <script type="text/javascript" src="templates/default/js/mustache.js"></script>
...
.navbar-nav .dropdown-menu>li>a, .navbar-nav .dropdown-menu>li>a:focus {
	...
	.indicator-false {cursor:auto !important;}
.footer {
    position: inherit;
    bottom:0;
    width:100%;
		...
		<span class="missing-email-response"></span>
                ...
                <form>
                    <div class="form-group">
                        <input type="email" id="missing-email" class="form-control" placeholder="Email Address" required>
                    </div>
                    <button type="submit" class="btn btn-primary send-email  ladda-button" data-expand="right">Verify</button>
                </form>
            </div>
        </div>
    </div>
...
<li
                            class="dropdown"><a href="#" class="contestLink" data-toggle="tk-modal-demo" data-template="contest-modal"
       data-modal-options="slide-down" data-targetId="2"><i class="fa fa-fw fa-plus"></i> Participate</a>
                        </li>
                    <?php } ?>
                </ul>
            </div>
...
<script src="templates/default/js/vendor-forms.js"></script>
<script src="templates/default/js/vendor-media.js"></script>
<script src="templates/default/js/vendor-player.min.js"></script>
...
<script src="templates/default/js/module-player.min.js"></script>
<!-- <script src="js/module-timeline.min.js"></script> -->
<script src="templates/default/js/module-chat.min.js"></script>
	...
	<?php } ?>
	 $("#btn1").click(function(event){
		...
		if(_email.length == 0 ){
		    ...
		    $("#password2").focus();$("#password2").parent().addClass('has-error');
			...
			$.ajax({
		            ...
		            success: function (data, textStatus, error) {
		                if (data.ErrorCode == 0
		                    ...
		                    $("#password2").removeClass('has-error');$("#password1").removeClass('has-error');$("#email").removeClass('has-error');
...
$(".change-password").click(function(event){
		...
		if(_oldpass.length == 0 || _oldpass.length<=6 ){
		    ...
		    $("#password1").focus();$("#password1").parent().addClass('has-error');
		    showMessage('New password can not be same as your last password.','danger');
		} else{
		    success = true;
		}
		if(success) {
		    $("#password2").removeClass('has-error');$("#password1").removeClass('has-error');$("#oldpassword").removeClass('has-error');
