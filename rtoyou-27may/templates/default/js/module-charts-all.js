...
(function ($) {
    ...
    charts.chart_lines_fill_nopoints_3 =
    {
	    ...
	    var _this = this;
	    $.ajax({
		url: 'review/dailyreviews',
		method: 'POST',
		data: {subcat:1},
		success: function (data1, textStatus, error) {
			var dat= JSON.parse(data1.data);	
			// make chart
			    _this.plot = $.plot(
				wrapper,
				[ {
...
(function ($) {
    ...
    charts.chart_pie =
    {
        // chart data
        data: [
            {label: "5 Stars", data: 40,color:'#8bbf61'},
            {label: "4 Stars", data: 45,color:'#26a69a'},
            {label: "3 Stars", data: 5,color:'#4193d0'},
            {label: "2 Stars", data: 8,color:'#ab7a4b'},
            {label: "1 Star", data: 20,color:'#bd362f'}
        ],
...
(function ($) {
	...
	var _data = [];
var _this = this;
	var _subcat = this.data("subcat");
	$.ajax({
		url: 'review/startrend',
		method: 'POST',
		data: {subcat:_subcat},
		dataType: 'json',
		success: function (data, textStatus, error) {
			 _data = data.data;
			if(_data.length == 0) {
				_this.html('<h3> No Data Available</h3>');
			} else {
			new Morris.Bar({
			    barColors: [ config.skins[ skin ][ 'primary-color' ]],
			    element: _id,
			    xLabelMargin: 10,
			    data: _data,
			    gridTextColor: colors[ 'default-color' ],
